...
// Initialize Theme
function initTheme() {
    const savedTheme = localStorage.getItem('wikelo_theme') || 'dark';
    ...
    document.getElementById('theme-toggle-btn').addEventListener('click', () => {
        ...
        localStorage.setItem('wikelo_theme', targetTheme);
...
function loadState() {
    const saved = localStorage.getItem('wikelo_bible_state');
    if (saved) {
        try {
...
// Update quantity on manual input change
function onQtyChange(inputElement) {
    const itemName = inputElement.dataset.item;
    let target = Math.max(0, parseInt(inputElement.value) || 0);
...
function renderCatalog() {
    ...
    for (const [category, items] of Object.entries(WIKELO_DATABASE)) {
        ...
        const filteredItems = items.filter(item => {
            const matchesSearch = item.name.toLowerCase().includes(searchVal) || 
                                  item.mission.toLowerCase().includes(searchVal) || 
                                  item.requirements.some(r => r.name.toLowerCase().includes(searchVal));
        ...
        bookHeader.innerHTML = `
            <span class="book-icon">${config.icon}</span>
            <h2 class="book-title">${config.title}</h2>
            <span class="book-subtitle">${config.sub}</span>
        `;
        ...
        filteredItems.forEach(item => {
            ...
            const reqsHtml = item.requirements.map(req => {
                ...
                return `
                    <li class="req-item ${isMet ? 'satisfied' : 'unsatisfied'}">
    ...
    document.getElementById('global-completed').innerText = `${totalCompleted} / ${totalItemsCount}`;
    const globalPct = totalItemsCount > 0 ? Math.round((totalCompleted / totalItemsCount) * 100) : 0;
    document.getElementById('global-percentage').innerText = `${globalPct}%`;
...
function bindEvents() {
    ...
    const txtArea = document.getElementById('backup-textarea');
    const copyBtn = document.getElementById('copy-btn');
    const importBtn = document.getElementById('import-btn');
    
    if (backupBtn && modal && closeModal) {
        backupBtn.addEventListener('click', () => {
            txtArea.value = btoa(unescape(encodeURIComponent(JSON.stringify(appState))));
    ...
    if (importBtn) {
        importBtn.addEventListener('click', () => {
            ...
            if (!raw) return alert("Please paste a backup code");
            try {
